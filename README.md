...
- затем отдельно загружает папку `public/` без удаления файлов на сервере
- папка `vendor/` загружается только если изменились `composer.json` или `composer.lock`
- после этого инкрементально загружает остальные изменившиеся файлы на FTP
...
- `FTP_PORT` — необязательно, порт FTP/FTPS, по умолчанию `21`
- `FTP_PROTOCOL` — необязательно, `ftp` или `ftps`, по умолчанию `ftp`

Папка `public/` загружается отдельным шагом через `lftp mirror -R` без удаления файлов на сервере. Это сделано специально, чтобы существующие папки и файлы в `public` не удалялись workflow ни при каких обычных деплоях.

Из этого upload-only шага исключены `public/storage`, `public/contentFiles`, `public/contentImages`, `public/logo` и `public/hot`, чтобы не трогать пользовательские и служебные данные.

Папка `vendor/` не участвует в обычном FTP sync. Она загружается отдельным шагом только при изменении `composer.json` или `composer.lock`, а также может быть принудительно загружена при ручном запуске workflow через параметр `upload_vendor`.
