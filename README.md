...
- затем полностью заменяет удалённую папку `public/build`
- после этого загружает остальные изменившиеся файлы на FTP
...
Папка `public/build` деплоится отдельным шагом с полной очисткой удалённой директории перед загрузкой. Это нужно, чтобы старые хешированные файлы из `public/build/assets` не накапливались на сервере после новых сборок.
