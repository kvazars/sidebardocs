			...
			<div v-for="val in fileData" :key="val">
				...
				<div v-if="val.type == 'gallery'" class="my-4 p-2 border">
					<div v-if="val.data.style == 'slider'">
						<div :id="'carousel' + val.id" class="carousel slide carousel-dark">
							<div class="carousel-indicators">
								<button v-for="(url, key) in val.data.files" :key="key" type="button" data-coreui-target
									:data-bs-target="'#carousel' + val.id" :data-bs-slide-to="key"
									:class="{ active: key == 0 }" aria-current="true"></button>
							</div>
							<div class="carousel-inner">
								<div class="carousel-item text-center" v-for="(url, key) in imgs[val.id]" :key="key"
									:class="{ active: key == 0 }">
									<img :src="url" class="img-fluid" style="max-height: 400px !important"
										:alt="'slide' + key" />
								</div>
							</div>
							<button class="carousel-control-prev" type="button" :data-bs-target="'#carousel' + val.id"
								data-bs-slide="prev">
								<span class="carousel-control-prev-icon" aria-hidden="true"></span>
								<span class="visually-hidden">Previous</span>
							</button>
							<button class="carousel-control-next" type="button" :data-bs-target="'#carousel' + val.id"
								data-bs-slide="next">
					...
					<div v-else class="row">
						<div v-for="(url, key) in imgs[val.id]" :key="key"
							class="pic col-lg-3 col-md-4 col-sm-6 col-12 mb-4">
							<div class="img-thumbnail h-100 d-flex">
								<img :src="url" class="img-fluid d-block" @click="() => showImg(val.id, key)" />
							</div>
						</div>
					</div>

					<vue-easy-lightbox :visible="visibleRef[val.id]" :imgs="imgs[val.id]" :index="indexRef[val.id]"
						@hide="onHide(val.id)"></vue-easy-lightbox>
				...
				<div v-if="val.type == 'code'" class="my-4">
					<VCodeBlock :code="val.data.code" highlightjs label="Пример кода" :lang="val.data.language"
						theme="monokai" persistent-copy-button />
				</div>
				<div v-if="val.type == 'image'" class="editorImageBlock text-center my-4" :class="{
					...
					<img :src="imgs[val.id]" alt="" class="editorImage text-center" />
						...
						<tbody>
							<tr v-for="(tRow, num) in val.data.content" :key="num">
								...
								<th v-if="num == 0 && val.data.withHeadings" v-for="tHeader in tRow" :key="tHeader"
									v-html="tHeader"></th>
								<!-- eslint-enable -->
								<td v-else v-for="tText in tRow" :key="tText" v-html="tText"></td>
							</tr>
						</tbody>
					</table>
				</div>
				...
				<ul class="my-4" v-if="val.type == 'list' && val.data.style == 'unordered'">
					<li v-for="item in val.data.items" :key="item" v-html="item"></li>
				</ul>
		...
		<div class="position-fixed squared" v-if="datasend">
			<div class="dropdown">
				...
				<ul class="dropdown-menu">
					<li v-if="content">
						<router-link class="dropdown-item" v-if="
							auths.id == content.tree.user_id ||
							auths.role == 'admin'
						" :to="{ name: 'EditFile', params: { id: id } }">Редактировать
